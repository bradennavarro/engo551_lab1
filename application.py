...
def reviews(book_id, user_id):
    ...
    if request.method == "POST":
        ...
        db.execute(text('INSERT INTO reviews (book_id, user_id, review) VALUES (:book_id, :user_id, :review)'),
                   {"book_id": book_id, "user_id": user_id, "review": str(review)})
        db.commit()
        searches = db.execute(text("SELECT * FROM reviews JOIN users ON users.id = reviews.user_id WHERE book_id = :book_id"),
                          {"book_id":book_id}).fetchall()
        return render_template("reviews.html",book_id=book_id,user_id=user_id,book=book,searches=searches,username=user.username)
    else:
        return render_template("reviews.html",book_id=book_id,user_id=user_id,book=book,searches=searches,username=user.username)
